...
for i,line in enumerate(f):
    ...
    if '#nsample' in words:
        ...
        print 'DES Y1 total chain size: ', nstop
        print 'DES Y1 burn-in: ', nbegin
    if '#log_z' in words:
...
print 'DES Y1 chain size after burn-in = ', len(ch), '\n'
f.close()
c.add_chain(ch,parameters=names,name=r'\textbf{DES Y1}',weights=w)
...
print 'DES Y1 + GW total chain size: ', nstop
print 'DES Y1 + GW burn-in: ', nbegin
...
w=np.array(np.exp(w))
ch=np.array(ch)
print 'DES Y1 + GW chain size after burn-in = ', len(ch)
f.close()
c.add_chain(ch,parameters=names,name=r'\textbf{DES Y1 + GW}',weights=w)
